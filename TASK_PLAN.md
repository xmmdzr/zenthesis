...
## Milestone Update (2026-03-03 21:18:02 CST) - PostgreSQL 基线切换
- 目标：将仓库从 SQLite 运行假设切到 PostgreSQL 生产基线。
- 完成项：
  - [x] `prisma/schema.prisma` 数据源 provider 切换为 `postgresql`
  - [x] `.env.example` 使用 PostgreSQL `DATABASE_URL`/`DIRECT_URL` 模板
  - [x] `src/lib/db.ts` 增加 `DATABASE_URL` 缺失时 fail-fast 防呆
  - [x] `npm run lint`、`npm run test`、`npm run build` 全通过
- 风险：部署环境若未配置有效 `DATABASE_URL`，服务启动会直接报错（预期防呆行为）。
- 下一入口：在目标环境执行 schema 同步（`prisma db push` 或迁移流程）并验证注册/登录链路。
